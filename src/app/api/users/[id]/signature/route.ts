...
export async function DELETE(
  ...
  try {
    ...
    if (roleError || !callerData) {
      return NextResponse.json(
        { error: "Unauthorized: Cannot verify user role" },
        { status: 403 },
      );
    }
    ...
    if (callerData.role !== "moderator") {
      return NextResponse.json(
        { error: "เฉพาะ Moderator เท่านั้นที่สามารถลบลายเซ็นได้" },
        { status: 403 },
      );
    }
    ...
    const { data: targetUser, error: targetError } = await supabaseAdmin
      .from("users")
      .select("id, email, name, role, signature_url")
      .eq("id", id)
      .single();
    ...
    try {
      await writeUserAudit(
        supabaseAdmin,
        {
          ...
          employee_id: callerData.employee_id,
        },
        id === user.id ? "UPDATE_PROFILE" : "UPDATE_USER",
        "ลบลายเซ็นผู้ใช้",
        {
          id: targetUser.id,
        ...
        {
          signature_url: {
            old: targetUser.signature_url ? "[signature]" : null,
            new: null,
          },
        },
      );
    } catch (auditError) {
      console.error("Signature deleted but audit logging failed:", auditError);

      return NextResponse.json(
        {
          error: "Signature was deleted, but audit logging failed",
        },
        { status: 500 },
      );
    }
