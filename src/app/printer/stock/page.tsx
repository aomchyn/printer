...
export default function StockPage() {
    ...
    const fetchTransactions = async () => {
        try {
            ...
            Swal.fire({ icon: 'error', title: 'ผิดพลาด', text: 'ไม่สามารถดึงข้อมูลสต็อคกระดาษได้' })
        } finally {
            setIsLoading(false)
        }
    }
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 max-w-sm w-full text-center">
                    ...
                    <p className="text-gray-500 text-sm mb-6">เฉพาะ Moderator และ Assistant Moderator เท่านั้นที่สามารถเข้าถึงการจัดการสต็อคกระดาษได้</p>
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!cleanQty || cleanQty <= 0) {
            Swal.fire({ icon: 'warning', title: 'ข้อมูลไม่ถูกต้อง', text: 'กรุณาระบุจำนวนกระดาษที่รับเข้าให้ถูกต้อง' })
            return
        }
        setSaving(true)
        try {
            ...
            if (error) throw error
            await logAction('STOCK_IN', { paperType, qty: cleanQty, description })
            Swal.fire({ icon: 'success', title: 'บันทึกสำเร็จ', timer: 1200, showConfirmButton: false })
            setQty(''); setDescription('')
            fetchTransactions()
        } catch (error) {
            Swal.fire({ icon: 'error', title: 'ผิดพลาด', text: (error as Error).message || 'ไม่สามารถบันทึกข้อมูลได้' })
        } finally {
            setSaving(false)
        }
    }
    ...
    const handleDelete = async (tx: PaperTransactionInterface) => {
        ...
        const impactLabel = tx.transaction_type === 'IN' ? 'ยอดสต็อคจะลดลง' : 'ยอดสต็อคจะเพิ่มขึ้น'
        ...
        try {
            const { error } = await supabase.from('paper_transactions').delete().eq('id', tx.id)
            if (error) throw error
            await logAction('STOCK_DELETE', { id: tx.id, paperType: tx.paper_type, qty: tx.qty, type: tx.transaction_type })
            Swal.fire({ icon: 'success', title: 'ลบสำเร็จ', timer: 1200, showConfirmButton: false })
            fetchTransactions()
        } catch {
            Swal.fire({ icon: 'error', title: 'ผิดพลาด', text: 'ไม่สามารถลบรายการได้' })
        }
    }
            ...
            <div className="bg-white/90 backdrop-blur-sm border-b border-gray-200 px-4 py-3 flex items-center justify-between gap-3 sticky top-0 z-30">
                <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                        <h1 className="text-[#0f1e3d] font-bold text-[16px] md:text-[18px] leading-tight tracking-wide truncate">จัดการสต็อคกระดาษ A3</h1>
                        <p className="text-slate-400 text-[12px] md:text-[13px] hidden sm:block truncate">รับเข้า · ตรวจเช็คประวัติการเบิกจ่าย</p>
                    </div>
                </div>
            </div>
            ...
            <div className="p-3 sm:p-5 max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="flex flex-col gap-4 lg:col-span-1">
                    <div className="bg-white border border-[#dde8f5] rounded-2xl p-5 shadow-sm">
                        <h2 className="text-[13px] font-black text-[#0f1e3d] mb-4">รับเข้ากระดาษ (Stock In)</h2>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block mb-1.5 text-[11.5px] font-semibold text-slate-500 uppercase tracking-wider">ประเภทกระดาษ</label>
                                <select className={`${inputCls} cursor-pointer`} value={paperType} onChange={e => setPaperType(e.target.value)}>
                                    {PAPER_TYPES.map(type => <option key={type} value={type}>{type}</option>)}
                                </select>
                            </div>
                            <div>
                                <label className="block mb-1.5 text-[11.5px] font-semibold text-slate-500 uppercase tracking-wider">จำนวน (แผ่น)</label>
                                <input type="number" min="1" className={inputCls} placeholder="เช่น 500" value={qty}
                                    onChange={e => setQty(e.target.value)} onWheel={e => e.currentTarget.blur()} required />
                            </div>
                            ...
                            <button type="submit" disabled={saving}
                                ...
                                <Plus className="w-4 h-4" /> {saving ? 'กำลังบันทึก...' : 'บันทึกรับเข้าสต็อค'}
                            </button>
                        </form>
                    </div>

                    <div className="bg-gradient-to-br from-[#0f1e3d] to-[#152a54] border border-[#0f1e3d] rounded-2xl p-5 shadow-sm">
                        <h3 className="text-[11px] font-black text-blue-200 uppercase tracking-widest mb-3">สต็อคกระดาษคงเหลือ</h3>
                        <div className="grid grid-cols-2 gap-2.5">
                            {balances.map(b => (
                                <div key={b.type} className="bg-white/5 border border-white/10 rounded-xl p-2.5 text-center">
                                    <span className="block text-[10px] text-blue-200/70 mb-1 truncate">{b.type}</span>
                                    <span className={`block text-lg font-bold ${b.balance < 2000 ? 'text-amber-400' : 'text-emerald-400'}`}>
                        ...
                        {isLoading ? (
                            ...
                            <div key={tx.id} className={`group flex items-center justify-between gap-3 bg-white border border-[#dde8f5] border-l-2 ${tx.transaction_type === 'IN' ? 'border-l-emerald-400' : 'border-l-rose-400'} rounded-xl px-3.5 py-3 hover:shadow-md transition-all`}>
                                <div className="flex items-center gap-2.5 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        <div className="text-[12.5px] font-bold text-[#0f1e3d] truncate">{tx.paper_type}</div>
                                        <div className="text-[11px] text-slate-400 truncate">
                                            {tx.description || '-'} {tx.created_by ? `· ${tx.created_by}` : ''}
                                        </div>
                                    </div>
                                </div>
